...
import ati.lims.controller.ProductController;
import ati.lims.model.BatchInfo;
import ati.lims.model.MaterialInfo;
...
public class BatchRepository {
    ...
    public List<BatchInfo> findAll(String status) {
        ...
        return jdbcTemplate.query(
                "select id, product_id,fnc_material_nm(product_id) product_name, batch_no, lot_no, batch_size,"
                        ...
                        + " (select fnc_material_type_nm(material_type_id) from lims_material_infos where id=a.product_id) product_type  "
                        + " from lims_batch_infos a where  status=" + (status != null ? "'" + status + "'" : "status") + " "
    ...
    public void saveBatchInfos(BatchInfo info) {
        ...
        if (info.getId() == null) {
            jdbcTemplate.update(
                    "INSERT INTO lims_batch_infos(product_id,batch_no,lot_no, batch_size, unit_id, mfg_date, exp_date, shelf_life, status, remarks, "
                            + " company_id, created_by, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    info.getProductId(),info.getBatchNo(), info.getLotNo(), info.getBatchSize(),info.getUnitId(),
                    info.getMfgDate(), info.getExpDate(), info.getShelfLife(), "P", info.getRemarks(), user.getCompanyId(), user.getId(), time);
            jdbcTemplate.update(
            ...
            log.info("New Batch No Save: " + info.getBatchNo() +"Create By : "+user.getId());
        } else {
            jdbcTemplate.update(
                    ...
                    info.getMfgDate(), info.getExpDate(), info.getShelfLife(),info.getRemarks(), "P",
    ...
    class BatchInfoRowMapper implements RowMapper<BatchInfo> {
        ...
        public BatchInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            info.setProductType(rs.getString("product_type"));
